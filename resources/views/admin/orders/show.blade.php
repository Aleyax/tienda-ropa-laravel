                    ...
                    <div class="text-sm">
                        @if ($basket)
                            ...
                            @if ($basket->responsibleUser)
                                <span class="ml-2">Responsable:
                                    <strong>{{ $basket->responsibleUser->name }}</strong></span>
                            @endif
                        @else
                        ...
                        @if (!$canTransfer)
                            <div class="text-xs text-gray-500">
                                ...
                                @elseif((int) $basket->responsible_user_id !== (int) $meId)
                                    Vista de solo lectura — Responsable:
                                    <strong>{{ $basket->responsibleUser?->name }}</strong>.
                                @else
        ...
        (function() {
            ...
            if (input) {
                const deposited = parseFloat(
                    "{{ $order->shipping_amount !== null ? number_format((float) $order->shipping_amount, 2, '.', '') : 0 }}"
                ) || 0;
